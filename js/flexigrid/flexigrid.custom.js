...
/*
 * Customized by makinosuke @ 2009.5.3
 */
...
(function($){
	...
	$.addFlex = function(t,p)
	{
		...
		p = $.extend({
			 ...
			 onNoItems: false // by makinosuke @ 2009.3.22
		  }, p);
		...
		var g = {
			...
			addData: function (data) { //parse data
				...
				if (p.total==0)
					...
					if(p.onNoItems) p.onNoItems(); // by makinosuke @ 2009.3.22
					return false;
					}
...
// by makinosuke @ 2009.3.29 ---->
							var tr = $('<tr></tr>');
							if(i % 2 && p.striped){ tr.addClass('erow'); }
							if(row.id){ tr.attr('id', 'row' + row.id); }
							if(row.data){ $.data(tr.get(0), 'fcabixdata', row.data); }
// <---- by makinosuke @ 2009.3.29
							//add cell
...
//										var td = document.createElement('td');
										...
										tr.append($('<td></td>').attr('align', this.align).append(row.cell[idx])); // by makinosuke @ 2009.3.29
//										td.align = this.align;
...
//										td = null;
							...
							if ($('thead',this.gDiv).length<1) //handle if grid has no headers
							{
									...
									for (idx=0;idx<cell.length;idx++)
										{
										tr.append($('<td></td>').append(row.cell[idx])); // by makinosuke @ 2009.3.29
//										var td = document.createElement('td');
//										td.innerHTML = row.cell[idx];
//										$(tr).append(td);
//										td = null;
...
//				$('input[name=q],select[name=qtype]',g.sDiv).keydown(function(e){if(e.keyCode==13) g.doSearch()});
				$('input[name=q]',g.sDiv).add('select[name=qtype]',g.sDiv).keydown(function(e){if(e.keyCode==13) g.doSearch()}); // by makinosuke @ 2009.2.25 for Safari 3(win) x jQuery1.3
