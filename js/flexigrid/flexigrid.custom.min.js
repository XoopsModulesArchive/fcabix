...
{$('tr, a, td, div',t).unbind();$(t).empty();p.pages=1;p.page=1;this.buildpager();$('.pPageStat',this.pDiv).html(p.nomsg);if(p.onNoItems)p.onNoItems();return false;}
...
{var tr=$('<tr></tr>');if(i%2&&p.striped){tr.addClass('erow');}
if(row.id){tr.attr('id','row'+row.id);}
if(row.data){$.data(tr.get(0),'fcabixdata',row.data);}
$('thead tr:first th',g.hDiv).each
(function()
{var idx=$(this).attr('axis').substr(3);tr.append($('<td></td>').attr('align',this.align).append(row.cell[idx]));});if($('thead',this.gDiv).length<1)
{for(idx=0;idx<cell.length;idx++)
{tr.append($('<td></td>').append(row.cell[idx]));}}
...
if(p.qtype=='')p.qtype=sitems[0].name;$(g.sDiv).append("<div class='sDiv2'>Quick Search <input type='text' size='30' name='q' class='qsbox' /> <select name='qtype'>"+sopt+"</select> <input type='button' value='Clear' /></div>");$('input[name=q]',g.sDiv).add('select[name=qtype]',g.sDiv).keydown(function(e){if(e.keyCode==13)g.doSearch()});$('input[value=Clear]',g.sDiv).click(function(){$('input[name=q]',g.sDiv).val('');p.query='';g.doSearch();});$(g.bDiv).after(g.sDiv);}}
